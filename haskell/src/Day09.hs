...
parseStream_ (x:xs) l g i = case x of
  '!' -> parseStream_ (tail xs) l g i
  '{' -> if g then parseStream_ xs l g i else parseStream_ xs l g (i + 1)
  '<' -> parseStream_ xs l True i
  '>' -> parseStream_ xs l False i
  '}' -> if g then parseStream_ xs l g i else parseStream_ xs (i:l) g (i - 1)
...
removeGarbage_ (x:xs) s g = case x of
  '!' -> removeGarbage_ (tail xs) s g
  '<' -> removeGarbage_ xs (if g then s + 1 else s) True
  '>' -> removeGarbage_ xs s False
  _   -> removeGarbage_ xs (if g then s + 1 else s) g
